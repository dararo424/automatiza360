...
async function main() {
  ...
  const serviciosClinica = [
    ...
    },
    {
      ...
      description: 'Laboratorio clínico básico',
      duration: 20,
      price: 30000,
    },
    {
  ...
  for (const svc of serviciosClinica) {
    const existing = await prisma.service.findFirst({
      where: { tenantId: clinica.id, name: svc.name },
    });
    if (!existing) {
  ...
  for (const med of medicos) {
    let prof = await prisma.professional.findFirst({
      where: { tenantId: clinica.id, name: med.name },
    });
    if (!prof) {
    ...
    for (let day = 1; day <= 5; day++) {
      const existing = await prisma.schedule.findFirst({
        where: { professionalId: prof.id, dayOfWeek: day },
      });
      if (!existing) {
  ...
  console.log(
    `    Bot: bot@${slugClinica}.automatiza360.com / ${BOT_PASSWORD}`,
  );
  ...
  for (const svc of serviciosBeauty) {
    const existing = await prisma.service.findFirst({
      where: { tenantId: beauty.id, name: svc.name },
    });
    if (!existing) {
  ...
  for (const est of estilistas) {
    let prof = await prisma.professional.findFirst({
      where: { tenantId: beauty.id, name: est.name },
    });
    if (!prof) {
      prof = await prisma.professional.create({
